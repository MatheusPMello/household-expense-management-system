...
async def calculate_cycle_report(
    ...
    # Build resident breakdown
    residents: List[ResidentCycleBalance] = []
    # Include active persons or persons who have non-zero balance/splits
    for p in persons:
...
async def calculate_general_balance_report(
    ...
    for p in persons:
        ...
        if assigned > 0:
            compliance = round((paid / assigned) * 100.0, 2)
        else:
            compliance = 100.0
