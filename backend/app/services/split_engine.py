...
def calculate_splits(
    ...
    if split_type_upper == "EQUAL":
    ...
    elif split_type_upper == "PERCENTAGE":
        if not percentages:
        ...
        for pid, pct in percentages.items():
            ...
            base_sum += floor_val
            # Sort key: primary percentage descending, secondary fraction descending
            fractional_remainders.append((pct, fraction, pid))

        leftover_cents = total_amount_cents - base_sum
        # Sort descending by percentage, then fractional remainder, with person_id as tiebreaker
    ...
    elif split_type_upper == "WEIGHTED":
        if not weights:
